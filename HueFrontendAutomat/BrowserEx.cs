...
using HueFrontendAutomat;
namespace HueFrontendAutomat.Browser
{
	/*
	 https://msdn.microsoft.com/en-us/library/ee330730%28VS.85%29.aspx#browser_emulation
	 Browser Emulation may be set for runtime interoperability
	 */
	...
	public class WebBrowserEx : WebBrowser
	{
	    ...
	    public HtmlDocument GetWebBrowserDocument()
        {
        	HtmlDocument returnObject = null;
			if (InvokeRequired) {
				Invoke((MethodInvoker)(() => returnObject = this.Document));
			} else {
				returnObject = this.Document;
			}
	        return returnObject;
        }
	    ...
	    /// <returns>null if the frame could not be found or string.empty if found frames body is null</returns>
	    public string GetFramesBodyInnerHtmlByUrl(string url, bool textOnly = false)
        {
	    	string returnObject = "";
	        if (this.InvokeRequired)
	        {
	           Invoke((MethodInvoker)(() => returnObject = getSpecificFrameByUrl(url, this.Document.Window.Frames, textOnly)));
	        }
	        else
	        {
	        	returnObject = getSpecificFrameByUrl(url, this.Document.Window.Frames, textOnly);
	        }
	        return returnObject;
        }
